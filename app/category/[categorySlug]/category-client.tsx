...
import { Building2, Phone, ArrowRight, ChevronRight, MapPin, Loader2 } from 'lucide-react'
...
import { NativeAdLoader as NativeAd, BannerAdLoader as BannerAd } from '@/components/ads/ads-loader'
...
export default function CategoryClient({ categorySlug }: { categorySlug: string }) {
  ...
  useEffect(() => {
    async function loadCategoryBusinesses() {
      if (!category) {
        setLoading(false)
        return
      }
    ...
    return (
      <>
        ...
        <main className="bg-[#f8fafc] min-h-screen py-16 text-center">
          ...
          <p className="text-slate-500 mt-2">The requested category does not exist.</p>
        </main>
        <Footer />
      </>
    )
  ...
  const content = generateCategoryContent(categorySlug)
  const pageUrl = `${BASE_URL}/categories/${categorySlug}`
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <section className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              ...
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.filter(c => c.id !== categorySlug).slice(0, 6).map(cat => (
                  <Link
                    key={cat.id}
